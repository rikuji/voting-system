...
namespace Sandbox
{
    public class Counter
    {
        ...
        public double GetPercent(int total) => _percentage ?? (_percentage = Math.Round(Count * 100.00 / total, 2)).Value;
    ...
    public class CounterManager
    {
        ...
        public void AnounceWinner()
        {
            ...
            if (winners.Count == 1)
            {
                ...
                System.Console.WriteLine($"{winner.Name} Won!");
            }
            else
            {
                if (winners.Count != Counters.Count)
                {
                    var lowestAmountOfVotes = Counters.Max(x => x.Count);
